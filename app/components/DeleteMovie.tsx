...
import { revalidateMovies } from "../actions/revalidate";
...
const DeleteMovie: React.FC<DeleteMovieProps> = ({
  ...
  console.log(movieId);
  const [deleteMovie, { loading }] = useMutation(DELETE_MOVIE, {
    refetchQueries: "all",
    onCompleted: async () => {
    ...
    onError: () => {
      message.error(`Failed to delete movie: ${message}`);
    },
  });

  const handleDelete = () => {
    deleteMovie({ variables: { deleteMovieId: movieId } });
  };
        ...
        <div className="bg-linear-to-br from-[#020617] via-[#020617] to-[#0f172a] overflow-hidden backdrop-blur-xl border border-white/10 shadow-2xl p-6 rounded-2xl">
          <div className="text-center mb-6">
            ...
            <h2 className="text-2xl font-black text-white tracking-tight mb-2">
              Delete <span className="text-red-500">Movie</span>
            </h2>
          ...
          <div className="bg-red-500/5 border border-red-500/10 rounded-xl p-3 mb-6 text-center text-red-200/80 text-xs">
            This action cannot be undone. This will permanently remove the movie
